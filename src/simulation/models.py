...
class Model:
    def __init__(self, val_date: Date, t: Periods, flag: bool, **kwargs):
...
class HullWhite(Model):
    ...
    def __init__(self, curve: Curve, MRV, MRS, val_date: Date, t: Periods, flag: bool, **kwargs):
        ...
        self.MRV = MRV
        self.MRS = MRS
        self.dt = self.curve.base.yearFraction(d1=self.val_date, d2=self.val_date + self.t,
    ...
    def sim(self, random):
        ...
        for i in range(0, l):
            dtenor = self.curve.base.yearFraction(d1=self.val_date, d2=self.curve.dates[i],
                                                  calendar=self.curve.calendar)
...
class LN(Model):
    def __init__(self, factor: Factor, curve: Curve, vol, val_date: Date, t: Periods, flag: bool, **kwargs):
        super().__init__(val_date=val_date, t=t, flag=flag, **kwargs)
        '''
        Excel implementation
...
class LnFwd(Model):
    ...
    def __init__(self, curve: Curve, vol, val_date: Date, t: Periods, flag: bool, **kwargs):
        super().__init__(val_date=val_date, t=t, flag=flag, **kwargs)
        if not isinstance(curve, Curve):
            raise ValueError("Curve is not a Curve class")
        self.curve = curve
